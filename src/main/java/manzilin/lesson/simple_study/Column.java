...
//}
//
//    static void {  column (KEY, tbl,  base1, base2)
//    local Mass = {}
//    local Optimum = {}
...
//            // Длинные вычисления
//    static void { Pressure_vapor(t, key)     // Определение давления насыщенных паров ур_Риделя-Планка-Мюллера
...
//    end
////
//    static void { BETA(Udin,ro, R1, Sc)     //         коэффициента массоотдачи
...
//    end
////
//        //static void { Concentracy_in_liquid(p1, p2)     // Концентрация в жидкой фазе
//// return (Mass.b*(Mass.Xn+p2*Mass.Y0*p1)-p2*Mass.alpha.Tn*p1-1+math.sqrt((1-Mass.b*Mass.Xn+p2*Mass.alpha.Tn*p1-p2*Mass.b*Mass.Y0*p1)^2+4*Mass.b*(Mass.Xn+p2*Mass.Y0*p1)))/(2*Mass.b);
////end
//    static void { sigma_pure(key,temp)      //         коэффициента поверхностного натяжения
...
//
//
//    static void { zapros_plate (key, num_plate_in_type )
//    Mass.N = base2[key].Info.PotokN; // число потоков на тарелке __zapros
//    Mass.n = base2[key].Param[num_plate_in_type].n/Mass.N; // число клапанов делится на потоки, предподсчет возможно сделать
//    Mass.Lper = base2[key].Param[num_plate_in_type].Lper; //_длина перегородки  или периметр слива // почему делился на два?? полупериметр или на потоки
...
//
//
//// Входные параметры, сделать запрос по ним
...
//// Сделать множественное кол-во параметров
...
////__zapros после определения номера параметров vk и nk
//
//// метод Тина и Калуса
////if base1[nk].VM then Mass.Vm.nk = base1[nk].VM else Mass.Vm.nk = 0.285*base1[nk].VC^(1.048) end
////if base1[vk].VM then Mass.Vm.vk = base1[vk].VM else Mass.Vm.vk = 0.285*base1[vk].VC^(1.048) end
...
//            // Подумать, как разделить процедуры до комментария и после
//            end
//// Дважды считать, требуется только то что ниже.
//    static void { column2 (KEY)
//    Mass.key=KEY
//if KEY=="UP"
//            // неясно как сделать функцией, та же самая констуркция получится что и здесь
////static void { calc_medium_vapor_K ()
//// end
//            then
...
//    end
//// Подумать, как разделить процедуры до комментария и после
//// Метод сканирования внедрить ниже
//// важные параметры Mass.hp, Mass.w0, Mass.d0
//    static void { column3 (KEY, plate_key, plate_num)
//  if KEY=="UP"  then
...
//    Mass.hp = i
////Mass.hp = tonumber(tbl )// высоса приемного порога_Изменить на метод сканирования
...
//
//// внутренний цикл метода сканирования // Нужно ли?
//            for  j = 0.0015, 0.01, 0.0005 do
//    Mass.rkl = j
////Mass.rkl = tonumber(tbl )//радиус отверстия клапана_Изменить на метод сканирования
...
//            end
//    end
// // Добавить множественную выгрузку. А также сортировку и сравнение, для выбора наиболее оптимального значения
////*********************************************Исключено из расчета**************************************************
//            //Mass.k1 = (1/2)*Mass.Pe+(1/2)*math.sqrt(Mass.Pe^2+4*Mass.kxv*Mass.Lzid^2/(Mass.V*Mass.Dt));
////Mass.k2 = (1/2)*Mass.Pe-(1/2)*math.sqrt(Mass.Pe^2+4*Mass.kxv*Mass.Lzid^2/(Mass.V*Mass.Dt));
////Mass.m = Mass.alpha.Tn/(1+(Mass.alpha.Tn-1)*Mass.Xn);
////Mass.y = Mass.Y0*(1-Mass.eta)+Mass.m*Mass.Xn*Mass.eta;
////Mass.xravn = -Mass.y/((Mass.alpha.Tn-1)*Mass.y-Mass.alpha.Tn);
////Mass.xk = (-Mass.k2*math.exp(.9*Mass.k2*Mass.Lzid)*math.exp(Mass.k1)*Mass.Xn+Mass.k2*math.exp(.9*Mass.k2*Mass.Lzid)*math.exp(Mass.k1)*Mass.xravn+Mass.k1*math.exp(.9*Mass.k1*Mass.Lzid)*math.exp(Mass.k2)*Mass.Xn-Mass.k1*math.exp(.9*Mass.k1*Mass.Lzid)*math.exp(Mass.k2)*Mass.xravn+Mass.xravn*Mass.k1*math.exp(.9*Mass.k1*Mass.Lzid)-Mass.xravn*Mass.k2*math.exp(.9*Mass.k2*Mass.Lzid))/(Mass.k1*math.exp(.9*Mass.k1*Mass.Lzid)-Mass.k2*math.exp(.9*Mass.k2*Mass.Lzid));
////Mass.ffi1 = (Mass.Xn-Mass.xk)/Mass.Xn;
////Mass.NN1 = math.log(Xd_nk_/Xf_nk_)/math.log(1/(1-Mass.ffi1));
////Mass.b = Mass.alpha.Tn-1;
////if KEY=="UP"
//            //then
////    Mass.kk= (Mass.R+1)/Mass.R
////else
//        //    Mass.kk= Mass.Gd*(Mass.R+1)/(Mass.Gd*Mass.R+Mass.Gf)
//            //end
////Mass.xkon1 = Concentracy_in_liquid( Mass.kk, Mass.eta)
////Mass.ffi2 = (Mass.Xn-Mass.xkon1)/Mass.Xn;
////Mass.xkon2 = Concentracy_in_liquid( Mass.kk, 1)
////Mass.ffi3 = (Mass.Xn-Mass.xkon2)/Mass.Xn;
////if KEY=="UP"
//            //then
////    Mass.NN2=math.log(Mass.Xd_nk_/Mass.Xf_nk_)/math.log(1/(1-Mass.ffi2));
////    Mass.NN3=math.log(Mass.Xd_nk_/Mass.Xf_nk_)/math.log(1/(1-Mass.ffi3))
//            //else
//            //    Mass.NN2=math.log(Mass.Xf_nk_/Mass.Xw_nk_)/math.log(1/(1-Mass.ffi2));
////    Mass.NN3=math.log(Mass.Xf_nk_/Mass.Xw_nk_)/math.log(1/(1-Mass.ffi3))
//            //end
////********************************************!Исключено из расчета!*************************************************
//
//    end
